 ...
 * real chat content.
 *
 *   SOURCE = 591 PX  ·  WINDOW ≤ SOURCE  ·  DEFAULT WINDOW = 518 PX
 *
 * 591×1280 is Telegram's own re-compression of the phone capture, and no
 * higher-resolution original exists. Source width and rendering width are different
 * numbers: a window that needs a different source width carries it in its own metadata
 * (`hx-01-request` is 522px). Nothing renders above 1:1 in CSS pixels.
 *
...
/**
 * REDACTION — the one canonical treatment for private detail.
 *
 * Solid --graphite. Never blurred, never a gradient, never textured, never Registration
 * Red, never a light patch that reads as a sticker, and never replaced with invented
 * content. It stays visibly a redaction and stays quieter than the evidence around it.
 */
const REDACTION = "#3a3b3e";

/**
 * CHROME REMOVAL is a different operation: it deletes Telegram's own furniture — date
 * pills, scroll affordances — by painting the surrounding colour, sampled from the image.
 * It is invisible by design. Pass an explicit colour for it.
 */
const bar = (x, y, w, h, colour = REDACTION) => ({
...
const jobs = [
  ...
  },
  {
    ...
    name: "lr-02-risk",
    src: "raw-01",
    // 932 is the last row of the bubble, measured off the source: the window stops on
    // the card's own bottom edge. The action row below it is a separate window (07.A).
    crop: { left: 14, top: 570, width: 518, height: 365 },
  ...
  },
  {
    ...
    // CHROME REMOVAL, not redaction: the two colours are sampled from the image so the
    // affordance disappears rather than becoming a visible mark
    ops: [
  ...
  },
  {
    ...
    src: "raw-08",
    // 261 is the bubble's first row, measured off the source. Starting exactly there
    // drops Telegram's floating date pill without a patch and without touching the
    // message: an earlier version painted over it and punched a light rectangle into
    // the bubble's own top edge.
    crop: { left: 14, top: 261, width: 518, height: 613 },
    // CHROME REMOVAL: the floating date pill overhangs the bubble's top band by ~8px.
    // Those pixels were already covered by Telegram's own overlay, so nothing of the
    // message is lost — the patch is the identical adjacent ground, sampled from the image.
    ops: [{ t: "bar", x: 218, y: 0, w: 134, h: 12, c: "#333333" }],
  },
  {
